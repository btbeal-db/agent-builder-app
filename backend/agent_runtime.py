"""Shared agent runtime: load a GraphDef, compile it, and run/stream it.

This module holds the load/compile/checkpointer/stream logic used by BOTH
deploy targets:

- **Model Serving** (``mlflow_model.py``): ``AgentGraphModel`` is a thin
  ``ResponsesAgent`` wrapper over ``load_agent``/``run_agent``/``stream_agent``.
- **Agents on Apps** (generated by ``app_deploy.py``): the generated
  ``agent/agent.py`` registers MLflow ``AgentServer`` ``@invoke()``/``@stream()``
  handlers that call the same three functions.

It must be importable standalone (no package-relative imports at module top)
because ``mlflow_model.py`` is loaded by MLflow via file path and inserts the
parent dir on ``sys.path`` before importing ``backend.*``.
"""
...
def _checkpointer_from_env() -> PostgresSaver | None:
    """Build a checkpointer from LAKEBASE_* env vars, or None if unset.

    Prefer dynamic token refresh (LAKEBASE_ENDPOINT/HOST/DATABASE), fall back
    to a static connection string (LAKEBASE_CONN_STRING).  Both Model Serving
    and the generated app pass these env vars identically.
    """
    ...
    if lb_endpoint and lb_host and lb_database:
        return _create_lakebase_checkpointer(lb_endpoint, lb_host, lb_database)

    conn_string = os.environ.get("LAKEBASE_CONN_STRING")
    if conn_string:
        return _create_connstring_checkpointer(conn_string)
...
# ── Compiled agent + entry points ────────────────────────────────────────


@dataclass
class CompiledAgent:
    """A compiled LangGraph agent plus the pieces run/stream need."""
...
def _resolve_invoke_input(agent: CompiledAgent, user_message: str, config: dict | None):
    ...
    """
    if agent.checkpointer and config:
        existing = agent.compiled_graph.get_state(config)
    ...
    return _build_initial_state(agent.graph_def, user_message)


def run_agent(agent: CompiledAgent, request: ResponsesAgentRequest) -> ResponsesAgentResponse:
    ...
    config = _build_config(agent.checkpointer, thread_id)
    invoke_input = _resolve_invoke_input(agent, user_message, config)

    result = agent.compiled_graph.invoke(invoke_input, config=config)
    ...
    interrupts = result.get("__interrupt__") or pending_interrupts(agent.compiled_graph, config)
    if interrupts:
        return _text_response(interrupt_value(interrupts[0]) or "Input needed")

    output, _ = filter_output(result, agent.graph_def)
    if not output:
...
def stream_agent(
    agent: CompiledAgent, request: ResponsesAgentRequest
    ...
    for chunk in agent.compiled_graph.stream(
    ...
    interrupts = pending_interrupts(agent.compiled_graph, config)
    ...
    if streamed_parts:
        ...
        if interrupts:
            ...
            full_text, _ = filter_output(final_state, agent.graph_def)
            if not full_text:
